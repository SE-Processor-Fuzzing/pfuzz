...
class Mutation:
    ...
    def crossover_func(self, chrom1: dict[str, str], chrom2: dict[str, str]) -> tuple[dict[str, str], dict[str, str]]:
        child1 = chrom1.copy()
        child2 = chrom2.copy()
    ...
    def mutation_func(self, chromosome: dict[str, str], template_config: dict[str, range]) -> dict[str, str]:
        child = chromosome.copy()
        random_key = random.choice(list(chromosome.keys()))
        child[random_key] = str(random.choice(template_config[random_key]))
        return child

    def fitness_func(self, chromosome: dict[str, str], func_generate: Callable[[dict[str, str]], None], func_run: Callable[[], int], desired_output: float) -> float:
    ...
    def population_sort(self, population: list[tuple[dict[str, str], float]], amount: int) -> list[tuple[dict[str, str], float]]:
        sorted_list = sorted(population, key=lambda x: x[1], reverse=True)
        return sorted_list[:amount]

    def mutation(self, population: list[tuple[dict[str, str], float]], template_config: dict[str, range], amount: int) -> None:
    ...
    def crossover(self, population: list[tuple[dict[str, str], float]], amount: int) -> None:
        random_chromosomes = random.sample(population, amount)
        for element in random_chromosomes:
    ...
    def genetic_func(self, population: list[tuple[dict[str, str], float]], template_config: dict[str, range], iterations: int, alive: int, reproduce: int, func_generate: Callable[[dict[str, str]], None], func_run: Callable[[], int], desired_output: float) -> list[tuple[dict[str, str], float]]:
